...
class TikTokDownloader:
    ...
    def download(self, url: str):
        ...
        try:
            self.logger.info(f"Detected TikTok URL: {url}")
            with yt_dlp.YoutubeDL({'quiet': True}) as ydl:
                info = ydl.extract_info(url, download=False)
                title = info.get('title', 'tiktok_video')
                safe_title = yt_dlp.utils.sanitize_filename(title)

            folder_name = "tiktok_" + datetime.now().strftime("%Y%m%d_%H%M%S")
            ...
            task_id = self.progress.add_task("Downloading TikTok video", total=100)
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ydl.download([url])
            self.progress.update(task_id, completed=self.progress.tasks[task_id].total)

            self.logger.info("TikTok video downloaded successfully.")
