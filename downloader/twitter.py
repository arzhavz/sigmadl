...
class TwitterDownloader:
    ...
    def download(self, url: str):
        ...
        try:
            self.logger.info(f"Detected Twitter URL: {url}")
            with yt_dlp.YoutubeDL({'quiet': True}) as ydl:
                info = ydl.extract_info(url, download=False)
                title = info.get('title', 'twitter_video')
                safe_title = yt_dlp.utils.sanitize_filename(title)

            folder_name = "twitter_" + datetime.now().strftime("%Y%m%d_%H%M%S")
            ...
            task_id = self.progress.add_task("Downloading Twitter video", total=100)
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ydl.download([url])
            self.progress.update(task_id, completed=self.progress.tasks[task_id].total)

            self.logger.info("Twitter video downloaded successfully.")
